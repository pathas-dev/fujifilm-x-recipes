...
interface ChatbotExampleMessagesProps {
...
const ChatbotExampleMessages = memo<ChatbotExampleMessagesProps>(
  ({ onSendMessage, hasAiResponses, isLoading }) => {
    const t = useTranslations("Chatbot");
    const cameraModel = useCameraStore(
      useShallow((state) => state.cameraModel)
    );
    ...
    const handleExampleClick = useCallback(
      ...
      [onSendMessage, cameraModel]
    );

    if (hasAiResponses || isLoading) {
      return null;
    }
